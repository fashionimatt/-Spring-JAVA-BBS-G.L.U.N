...
private int content_idx;
	...
	// SpringMVC에서는 클라이언트가 보낸 파일 데이터는 MultiPartFile이라는 객체로 만들어서 주입하려고 시도한다.
	// 여기에서는 content_file이 String 타입으로 설정되어있기에 오류가 발생한다.
	
	// 데이터베이스에 저장되어있는 파일 이름을 담을 변수
	private String content_file;
	
	// 브라우저가 보낸 파일 데이터를 담기 위해서 사용할 변수를 설정해준다.
